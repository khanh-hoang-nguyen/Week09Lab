    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (userservice.fieldValidation(email, firstname, lastname, password) == null) {
                switch (action) {
                    case "update": {
                        userservice.update(email, firstname, lastname, password, roleservice.getRoleID(role));
                    }
                    break;
                    case "create": {
                        List<User> users = userservice.getAllUser();
                        if (userservice.userValidation(users, email)) {
                            userservice.insert(email, firstname, lastname, password, roleservice.getRoleID(role));
                        } else {
